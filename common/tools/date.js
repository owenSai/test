...
export default  {
  ...
  formatDate(_time, _format){
    function getShowStr(_value) {
      if (_value < 10) {
        return "0" + _value
    ...
    return _format.replace("yyyy", year).replace("yy", year.toString().substr(2, 2)).replace("MM", getShowStr(month))
      .replace("dd", getShowStr(day)).replace("HH", getShowStr(hour))
      .replace("mm", getShowStr(minutes)).replace("ss", getShowStr(seconds));

  },
  //获取
  getComparedDay: function (someDays) {
    var t = new Date();
    var iToDay = t.getDate();
    var iToMon = t.getMonth();
    var iToYear = t.getFullYear();
    var newDay = new Date(iToYear, iToMon, (iToDay + someDays));
    return newDay;
  }
};
